...
{
	private static final BundleLogger	LOGGER	= BundleLoggerFactory.getLogger(HttpInboundAuthTokenTransport.class);
	...
	private String getToken()
	{
		...
		try (GeoEventHttpClient http = httpClientService.createNewClient())
		{
			...
			{
				String message = ((HttpTransportContext) context).getHttpRequest().getRequestLine().getUri() + " :  Request failed(" + statusLine.toString() + ")";
			...
			/*
			 * { "token":"9dfe42ce2368c55813d32b51fed692f7", "customerName":"ACMP", "username":"Svcgis" }
			 */
	...
	{
		LOGGER.debug("received at: " + new Date());
		...
		{
			String message = ((HttpTransportContext) context).getHttpRequest().getRequestLine().getUri() + " :  Request failed(Error parsing response.)";
